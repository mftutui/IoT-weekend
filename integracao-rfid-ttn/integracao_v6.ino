...
int hallSensorPin = 38;  

#define SEND_TAG_INTERVAL  60 // em segundos 
enum RFID_Commands
{
...
#define LAST_TAG_LIST_SIZE 15
#define INTERNAL_TAG_BUFFER 15
struct tag_ring_s
{
...
void loop() {
    ...
    loopRfid();   // loop da leitura rfid
    
    if(needSend){
        Serial.println("lora_in");
        os_runloop_once();
        needSend = false;
    }
    ...
    /*
     * dentro do loopRfid todas as funções necessárias para a leitura das tags são executadas
     * as tags serão reportadas quando elas são lidas e quando elas não são lidas durante o intervalo "interval"
     * isso significa que a tag não está mais próxima do leitor
     * e a cada minuto, definido em "sendTagInterval", a tag é portada tbm. 
     * 
     * As mensagens devem ser enviadas através da função 
     * sendTagLora
     * 
     * 
     * 
...
void SendTagLora(String epc)
{
    char tag[25];
       // tag[0 ~ 23] + codeCheck[24] + sensor[25]

    /*
     * codeCheck é a verificação do inicio da tag
     * 1 = ok, 0 = not ok
     * 
     * Sensor é o campo para a leitura do sensor hall
     * a leitura pode ser realizada conforme demostrado na linha abaixo
     * payload[25] = digitalRead(hallSensorPin);
     * 
     */

     
    epc.toCharArray(tag, sizeof(tag)+1);
    memcpy(payload, tag, 25);
    Serial.println(payload);

    /*
     * logica comparação tag com a verificação
     * exemplo abaixo
     */
//     if(verificação == verdadeira){
//          payload[24] = 1;
//     }else{
//          payload[24] = 0;      
//     }
    ...
    if(digitalRead(hallSensorPin) == LOW){
        payload[25] = 1; //cilindro esta presente
    }
    else{
        payload[25] = 0; //cilindro removido
    }
...
void trataTAG(struct tag_s t)
{
  ...
  else if (sameTAG == true && sameantenna == true) //Tag já na lista, renova o tempo
  {
    ...
    //Serial.println("sameTAG");

    
    long unsigned currentTime = millis();
    //Serial.println(lastSendTag);
    //Serial.println(sendTagInterval);
    //Serial.println(currentTime);
    
    if((currentTime - lastSendTag) > sendTagInterval){
      Serial.println("PERIODIC");
      String epc = getEpcByTag(internalTags[index_tag_on_list]);
      SendTagLora(epc);     
      //ENVIAR LORA STRING EPC
      // ENVIAR A TAG LIDA PEDIÓRICAMENTE, O PERIODO É DEFINIDO EM "sendTagInterval" EM SEGUNDOS 
      lastSendTag = millis();
    }
...
void PrintTAGS()
{
  ...
  for (int k = 0; k < INTERNAL_TAG_BUFFER; k++)
  {
    ...
    if (internalTags[k].readed == READED)
    {
      ...
      SendTagLora(epc);
      //ENVIAR LORA STRING EPC
      // QUANDO A TAG É LIDA PELA PRIMEIRA VEZ
    ...
    else if (internalTags[k].readed == JUSTLEFT)
    {
      ...
      SendTagLora(epc);
      // ENVIAR LORA STRING EPC
      // QUANDO A TAG NÃO É LIDA DURANTE O INTERVALO DE TEMPO DEFINIDO EM "unsigned int interval"
